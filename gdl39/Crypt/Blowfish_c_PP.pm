...
# modified to support reduced rounds
...
sub new
	...
	if (!defined $rounds) {$rounds = 16;};
	$h{"rounds"}=$rounds;
	my $keylen=length($key);
if(($keylen < 8) || ($keylen > 56)) {warn "Invalid key length";
				     return undef ;
};
...
sub crypt_block
	...
	if(!$d)
		...
		$r^=$self->{"p_boxes"}->[$self->{'rounds'}+1];
		}
	else
		{
		$l^=$self->{"p_boxes"}->[$self->{'rounds'}+1];
		my $i;
		for $i (1..$self->{'rounds'})
			{
			($r,$l)=ROUND($self,$l,$r,($self->{'rounds'}+1)-$i);
			}
		$r^=$self->{"p_boxes"}->[0];
		}
	return($r,$l);
